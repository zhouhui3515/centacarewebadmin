<?php 
	...
	$newhash = new PasswordStorage;
	$str1 = (string)$pass;
	$str2 = (string)$checkhash;
	
	if($checkuser == ''){
		echo "<script>alert('The username or password is invalid. Please try again.')</script>";
		header("refresh:0; url=index.php");
		exit();	
	}
	
	if($newhash->verify_password($str1,$str2)){
	
		if ($user!="admin" and $checkuser==$user){
		...
		elseif ($checkuser==$user and $user="admin") { 
		$_SESSION['username'] = $user;
		$_SESSION['password'] = $pass;
		header("Location: home.php");
        exit();
...
/* check the password with hashing */
